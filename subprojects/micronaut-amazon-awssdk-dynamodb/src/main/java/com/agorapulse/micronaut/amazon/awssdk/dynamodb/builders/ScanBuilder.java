...
public interface ScanBuilder<T> extends DetachedScan<T> {
    ...
    default ScanBuilder<T> filter(
        @DelegatesTo(type = "com.agorapulse.micronaut.amazon.awssdk.dynamodb.builders.RangeConditionCollector<T>", strategy = Closure.DELEGATE_FIRST)
        @ClosureParams(value = FromString.class, options = "com.agorapulse.micronaut.amazon.awssdk.dynamodb.builders.RangeConditionCollector<T>")
