...
.Gradle (AWS SDK 2.x)
----
compile 'com.agorapulse:micronaut-amazon-awssdk-s3:{project-version}'
----
...
<4> Create `S3Client` client using the LocalStack configuration
<5> Create `S3Presigner` client using the LocalStack configuration
<6> Prepare the application context with default configuration
...
<3> Create `S3Client` client using the LocalStack configuration
<4> Register `S3Client` service running against LocalStack
