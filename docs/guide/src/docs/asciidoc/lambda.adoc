...
.application.yml
----
include::{root-dir}/subprojects/micronaut-amazon-awssdk-lambda/src/test/resources/application-test.yml[tags=configuration]
----
...
<3> The `LambdaClient` is populated automatically pointing to the Localstack test container
