...
<3> Each of the configuration can be access using `@Named('other') KinesisService` qualifier or you can define the configuration as `value` of `@KinesisClient('other')`
<4> For Kinesis listeners you should provide application name which defaults to `micronaut.application.name` if not present
<5> You can also provide the worker ID of the Kinesis worker
<6> This is the default stream to listen
<7> You can listen to multiple Kinesis streams by declaring the name of the configuration in the annotation such as `@KinesisListener("other")`
...
NOTE: The return value of the method is `PutRecordResponse` or `PutRecordsResponse`, but it can be always omitted and replaced with `void`.
...
Please, see
https://agorapulse.github.io/micronaut-amazon-awssdk/api/com/agorapulse/micronaut/amazon/awssdk/kinesis/KinesisService.html[KinesisService]
