...
.Gradle (AWS SDK 2.x)
----
compile 'com.agorapulse:micronaut-amazon-awssdk-dynamodb:{project-version}'
----
...
For AWS SDK 1.x use classes from `com.agorapulse.micronaut.aws.dynamodb` package. It uses
https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DynamoDBMapper.html[DynamoDBMapper] for entity mappings.

For AWS SDK 2.x use classes from `com.agorapulse.micronaut.amazon.awssdk.dynamodb` package. It uses https://docs.aws.amazon.com/sdk-for-java/latest/developer-guide/examples-dynamodb-enhanced.html[DynamoDB Enhanced Client] for entity mappings.
...
.Groovy (AWS SDK 2.x)
----
include::{root-dir}/subprojects/micronaut-amazon-awssdk-dynamodb/src/test/groovy/com/agorapulse/micronaut/amazon/awssdk/dynamodb/DefaultDynamoDBServiceSpec.groovy[tags=testcontainers-header]
include::{root-dir}/subprojects/micronaut-amazon-awssdk-dynamodb/src/test/groovy/com/agorapulse/micronaut/amazon/awssdk/dynamodb/DefaultDynamoDBServiceSpec.groovy[tags=testcontainers-setup]
    }
...
<4> Create `DynamoDbClient` client using the LocalStack configuration
<5> Register the client using LocalStack to the application context
<6> Register the enhanced client using LocalStack to the application context
<7> Obtain the declarative service
<8> Obtain `DynamoDBService` for particular DynamoDB entity

[source,java,indent=0,role="secondary"]
.Java (AWS SDK 2.x)
----
include::{root-dir}/subprojects/micronaut-amazon-awssdk-dynamodb/src/test/groovy/com/agorapulse/micronaut/amazon/awssdk/dynamodb/DeclarativeServiceTest.java[tags=testcontainers-setup]
    @Test
...
<3> Create `DynamoDbClient` client using the LocalStack configuration
<4> Register the client using LocalStack to the application context
<5> Register the enhanced client using LocalStack to the application context
